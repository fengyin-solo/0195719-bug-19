...
const CONFIG = {
    ...
    // 帮助文本（素材库"?"说明与参数面板使用同一套说法，均与实际光路计算一致）
    HELP_TEXTS: {
        convex: '凸透镜（球面）：中间厚、边缘薄，能把平行光会聚到焦点。球面表面使边缘光线偏折偏强，会聚点比中心光线略近，存在约 13% 的球差。',
        concave: '凹透镜：中间薄、边缘厚，平行光通过后向外发散，反向延长线交于镜前的虚焦点。',
        plano: '平面透镜：两个表面互相平行，薄透镜近似下光线方向不发生偏折。',
        aspheric: '非球面透镜：表面曲率从中心到边缘逐渐变平，补偿球面边缘的过度偏折。按折射率与波长计算后，不同高度的平行光都会聚到同一焦点，球差接近零。',
        lowDispersion: '低色散镜片（ED玻璃）：阿贝数高，红、绿、蓝各波长的折射率差异小，各颜色焦点几乎重合，成像更清晰无彩边。',
        highIndex: '高折射率镜片：折射率高、聚光能力强，同样曲率下焦距更短、镜片可做得更薄；色散相对明显。',
        refractiveIndex: '折射率：数值越大，相同曲率下偏折越强、焦距越短。折射率随波长变化（蓝光大、红光小），这就是色散的原因。',
        curvature: '弧度：调节透镜表面的弯曲程度。弧度越大焦距越短，球面透镜的球差也越容易观察。'
    },
