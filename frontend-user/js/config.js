...
const CONFIG = {
    ...
    // 帮助文本
    HELP_TEXTS: {
        convex: '凸透镜（球面）：中间厚、边缘薄，能把光线会聚到焦点。但球面表面会让边缘光线偏折过度，提前会聚，产生球差（图中橙色小点是边缘光线的会聚位置）。',
        concave: '凹透镜：中间薄、边缘厚，使光线向外发散，其反向延长线交于虚焦点（空心圆点）',
        plano: '平面透镜：两面平行，光线垂直入射时方向不变',
        aspheric: '非球面透镜：表面曲率从中心到边缘逐渐变平缓，恰好补偿球面透镜的球差，让边缘光线和中心光线都穿过同一个焦点。其近轴焦距与同参数凸透镜相同，焦点处不再有分离的橙色小点。',
        lowDispersion: '低色散镜片（ED玻璃）：阿贝数高，按波长计算的红/绿/蓝折射率差异很小，三色焦点几乎重合，成像无彩边。开启工具栏「色散」开关可对比观察。',
        highIndex: '高折射率镜片：光焦度大，同样曲率下焦距更短、聚光能力更强，镜片可以做得更薄',
        refractiveIndex: '折射率：数值越大，光焦度越大、光线偏折越明显。偏折按 n(λ) 随波长变化计算：蓝光折射率最大、红光最小，这就是色散。',
        curvature: '曲率（弧度）：滑块数值越大，光焦度越大、焦距越短；球面凸透镜曲率越大球差也越明显'
    },
